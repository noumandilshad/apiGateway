    ...
    return (req, res, next) => {
      if(req.query.id_token==undefined){
        if(req.headers.cookie){
          ...
          const reqUrl =  `https://node-oidc-provider-examples-ykwufwclzr.now.sh/auth?issuerHost=https://node-oidc-provider-examples-ykwufwclzr.now.sh&client_id=foobar&redirect_uri=https://example.com&scope=openid&response_type=id_token+token&nonce=1122`
          res.redirect(reqUrl);
        }
      }
