    ...
    return (req, res, next) => {
      if(req.query.id_token==undefined){
        if(req.headers.cookie){
          ...
          const reqUrl =  `https://node-oidc-provider-examples-oqxyxpbxyh.now.sh/auth?issuerHost=https://node-oidc-provider-examples-oqxyxpbxyh.now.sh&client_id=foobar&redirect_uri=https://node-oidc-provider-examples-oqxyxpbxyh.now.sh/callback&scope=openid&response_type=id_token+token&nonce=1122`
          res.redirect(reqUrl);
        }
      }
